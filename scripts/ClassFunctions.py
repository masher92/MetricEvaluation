...
class rainfall_analysis:
    ...
    def huff_quantile(self,dimensionless_event):
    ...
    def quintile_class(self,dimensionless_event):
...
#         self.metrics["ttp"] = [time_delta_minuts + np.float32((padded_precip.loc[event[0]:event[1]].idxmax() - event[0]).values / (60 * 10**9)) for event in events_list]
        # Duration [min]
...
#         # huff quantiles
#         self.metrics["huff_quantile"] = np.array([[self.huff_quantile(dimensionless_event)] for dimensionless_event in self.ts.dimensionless_events])

#         # quintile class
#         self.metrics["quintile_class"] = np.array([[self.quintile_class(dimensionless_event)] for dimensionless_event in self.ts.dimensionless_events])
...
#         self.metrics["third_class_CoM"] = np.array([[self.third_classification_CoM(dimentioneless_curve)] for dimentioneless_curve in self.ts.dimensionless_events])
