...
class precip_time_series:
    ...
    def read_raw_data_as_pandas_df(self, raw_data_file_path):
        ...
        # Save ID of station
        if 'SVK' in raw_data_file_path:
            station_id = raw_data_file_path[54:58]
        else:
            station_id = raw_data_file_path[50:56]            
...
#     def init_events(self, data_path):
        
#         with open(f'/nfs/a319/gy17m2a/MetricEvaluation/DanishRainDataPickles/{data_path}.pkl', 'rb') as f:
#                        five_min_pickle = pickle.load(f)
#         five_min_events = five_min_pickle.events
#         print(len(five_min_events))
#         precip = self.data

#         events=[]

#         for i in range(len(five_min_events)):
#             start_time, end_time = five_min_events[i]
#             # Slice the data between start_time and end_time
#             event_data_30min = precip.loc[start_time:end_time]
#             current_start = event_data_30min.index[0]
#             current_end = event_data_30min.index[-1]
#             current_start, current_end
#             events.append((current_start, current_end))
            
#         print(f"events after:{len(events)}")
#         ########################################        
...
#     def init_events(self, data_path, temp_res):
#         """
#         Match 5-min events to 30-min data, intelligently adjusting for rainfall presence,
#         with print statements to trace what's happening.
#         """
#         with open(f'/nfs/a319/gy17m2a/MetricEvaluation/DanishRainDataPickles/{data_path}.pkl', 'rb') as f:
#             five_min_pickle = pickle.load(f)
#         five_min_events = five_min_pickle.events
...
#         precip = self.data  # Assumed to be 30-min resolution with datetime index and 'Precip' column

#         events = []
#         original_indices = []

#         for i, (start_5m, end_5m) in enumerate(five_min_events):
#             # print(f"\nEvent {i}: 5-min start = {start_5m}, end = {end_5m}")

#             # Find 30-min bin at or before 5-min start
#             bin_start_candidates = precip[precip.index <= start_5m]
#             if bin_start_candidates.empty:
#                 #print("  ⚠️ No valid 30-min bin before start — skipping.")
#                 continue
#             bin_start = bin_start_candidates.index[-1]
#             #print(f"  ↪ Closest 30-min bin before start: {bin_start} (rainfall = {precip.loc[bin_start, 'precipitation (mm/min)']})")

#             # Move forward if no rainfall in bin_start
#             if precip.loc[bin_start, 'precipitation (mm/min)'] == 0:
#                 #print("  💤 Bin has no rain, searching forward...")
#                 later_bins = precip[precip.index > bin_start]
#                 for idx in later_bins.index:
#                     if precip.loc[idx, 'precipitation (mm/min)'] > 0:
#                         #print(f"  ✅ Found later rainy bin: {idx}")
#                         bin_start = idx
#                         break
#                 else:
#                     #print("  ❌ No rainy bin found after start — skipping.")
#                     continue

#             # Find 30-min bin at or before 5-min end
#             bin_end_candidates = precip[precip.index <= end_5m]
#             if bin_end_candidates.empty:
#                 #print("  ⚠️ No valid 30-min bin before end — skipping.")
#                 continue
#             bin_end = bin_end_candidates.index[-1]
#             #print(f"  ↪ Closest 30-min bin before end: {bin_end} (rainfall = {precip.loc[bin_end, 'precipitation (mm/min)']})")

#             # Move backward if no rainfall in bin_end
#             if precip.loc[bin_end, 'precipitation (mm/min)'] == 0:
#                 #print("  💤 Bin has no rain, searching backward...")
#                 earlier_bins = precip[precip.index < bin_end][::-1]
#                 for idx in earlier_bins.index:
#                     if precip.loc[idx, 'precipitation (mm/min)'] > 0:
#                         #print(f"  ✅ Found earlier rainy bin: {idx}")
#                         bin_end = idx
#                         break
#                 else:
#                     print("  ❌ No rainy bin found before end — skipping.")
#                     continue

#             # Final sanity check
#             if bin_start > bin_end:
#                 #print(f"  ❌ Skipping: start ({bin_start}) is after end ({bin_end})")
#                 continue

#             #print(f"  ✅ Final 30-min event: start = {bin_start}, end = {bin_end}")
#             events.append((bin_start, bin_end))
#             original_indices.append(i)

#         self.events = events
#         self.original_event_indices = original_indices
#         #print(f"\n✅ Total matched 30-min events: {len(events)}")
...
class rainfall_analysis:
    ...
    def compute_rcg_interpolated(self, series, suffix):
        ...
        # Compute the weighted average of positions (without rounding)
        weighted_sum = np.sum(positions * series)

        # Interpolate: find the fractional centroid
    ...
    def compute_frac_in_quarters(self, series, suffix, interpolate=False):
        # Convert cumulative to intensity if needed
        if suffix in ['_dblnorm', '_norm']:
            series = np.diff(series, prepend=0)
        if suffix in ['_DMC_10', '_DMC_100']:
            series = series[1:]
        ...
        if interpolate:
            ...
            cumulative /= total  # Normalise cumulative to 0–1

            # Interpolate to find times at 0.25, 0.5, 0.75
            quarter_marks = [0.25, 0.5, 0.75]
            time_bounds = [0] + list(np.interp(quarter_marks, cumulative, time)) + [1.0]

            frac_in_quarters = []
            for i in range(4):
                # Get mask of timesteps in the quarter range
                mask = (time >= time_bounds[i]) & (time < time_bounds[i+1])
                if i == 3:  # Include final point in last bin
                    mask = (time >= time_bounds[i]) & (time <= time_bounds[i+1])
                rainfall = series[mask].sum()
                frac_in_quarters.append(round(rainfall / total * 100, 1))
        else:
            # Original fixed-index version
            n = len(series)
...
#         temp_res = np.timedelta64(30, 'm')
        ...
        self.metrics["third_ppr"] = np.select([
        self.metrics["peak_position_ratio"] < 0.4,
        (self.metrics["peak_position_ratio"] >= 0.4) & (self.metrics["peak_position_ratio"] <= 0.6),
        self.metrics["peak_position_ratio"] > 0.6],[0, 1, 2])
...
#             self.metrics[f"relative_amp_scaled{suffix}"] = [(np.max(vals := e.iloc[:, 0].to_numpy()[e.iloc[:, 0].to_numpy() > 0]) - np.min(vals)) / np.mean(vals) if np.any(e.iloc[:, 0].to_numpy() > 0) else np.nan for e in events]
            ...
            self.metrics[f"3rd_com{suffix}"] = np.array([self.third_CoM(e.iloc[:, 0].to_numpy(), suffix) for e in events]) 
