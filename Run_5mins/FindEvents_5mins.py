...
if pd.read_csv(f"/nfs/a319/gy17m2a/MetricEvaluation/{directory}/{file}").empty:
    print("The CSV file has no data rows.")
else:   
    ts = precip_time_series(f"/nfs/a319/gy17m2a/MetricEvaluation/{directory}/{file}")
    if len(ts.data[ts.data['precipitation (mm/hr)']<0]) >0:
        print("Not including, still has negatives")
        negatives.append(file)
    else:
        ts.pad_and_resample(f'{temp_res}min')
        ...
        if ts.events != None:
            ...
            all_events_df.to_csv(f"/nfs/a319/gy17m2a/MetricEvaluation/DanishRainData_Outputs/{temp_res}mins/All_events_{file}", index=False)