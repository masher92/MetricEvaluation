...
class precip_time_series:
    ...
    def read_raw_data_as_pandas_df(self, raw_data_file_path):
        ...
        # Run QC
        qc = pd.read_csv(home_dir + f"Other/station_exclusion_periods_from_climadb.csv")
    ...
    def init_events(self, threshold):
        ...
        precip_sum['precipitation (mm/min)'] = precip_sum['precipitation (mm/min)'].apply(lambda x: 0 if abs(x) < 1e-10 else x)
...
class rainfall_analysis:
    ...
    def calculate_skew_p(self, series, suffix, dt_minutes=5):
    ...
    def get_metrics(self):
        ...
        window = "30min"
        window_duration_hr = 30 / 60  # = 0.5 hours

        window_size = int(30 / temp_res)  # number of time steps in 30 minutes
        self.metrics["I30"] = np.array([e.rolling(window=window_size, min_periods=1).sum().max().values[0] / 0.5  # divide by 0.5 hours
...
#             "_norm": self.ts.normalised_events
        ...
        for suffix, events in event_sets.items():
            ...
            if suffix in ['_norm', '_dblnorm']:
                ...
                self.metrics[f"kurtosis{suffix}"] = np.array([kurtosis(np.diff(e.iloc[:, 0].to_numpy(), prepend=0), bias=False) for e in events])
            else:
